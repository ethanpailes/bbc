#ifndef BYTE_BLOCKS__JCDKPXHBORNGYDNLBFNZ
#define BYTE_BLOCKS__JCDKPXHBORNGYDNLBFNZ
#include <string.h>
#include <stdint.h>
#include <endian.h>
#include <stdlib.h>
...
typedef struct preq_part {
    int32_t partition;
    int32_t messages_len;
    message * messages;
} preq_part;

int preq_part_size(const preq_part const * b)
{
    int size = 8;
    int32_t messages_iter = 0;
    for(messages_iter = 0; messages_iter < b->messages_len; ++messages_iter) {
        size += message_size(b->messages + messages_iter);
    }
    return size;
}
int preq_part_pack(const preq_part const *src, char *tgt)
{
    size_t bytes_written = 0;
    *((int32_t*)(tgt + bytes_written)) = htobe32(src->partition); bytes_written += 4;
    *((int32_t*)(tgt + bytes_written)) = htobe32(src->messages_len); bytes_written += 4;
    int32_t messages_iter;
    for(messages_iter = 0; messages_iter < src->messages_len; ++messages_iter) {
        bytes_written += message_pack(&(src->messages[messages_iter]), (tgt + bytes_written));
    }

    return bytes_written;
}
int preq_part_unpack_new(preq_part *tgt, const char const *src)
{
    size_t bytes_consumed = 0;
    tgt->partition = be32toh(* ((int32_t*)(src + bytes_consumed))); bytes_consumed += 4;
    int32_t messages_iter = 0;
    tgt->messages_len = be32toh(* ((int32_t*)(src + bytes_consumed))); bytes_consumed += 4;
    tgt->messages = malloc(tgt->messages_len * sizeof(message));
    if (tgt->messages_len < 0) tgt->messages_len = 0;
    for(messages_iter = 0; messages_iter < tgt->messages_len; ++messages_iter) {
        bytes_consumed += message_unpack_new(&(tgt->messages[messages_iter]), (src + bytes_consumed));
    }
...
int preq_part_write(const preq_part *src, FILE *f)
{
    int ret;
    size_t blk_size = preq_part_size(src);
    char * buff = (char*) malloc(blk_size);
    if(!preq_part_pack(src, buff)) return false;
    ret = fwrite(buff, blk_size, 1, f);
    free(buff);
    return ret;
}
int preq_part_read_new(preq_part *tgt, FILE *f)
{
    size_t buff_len = 1024;
    size_t used = 0;
    char *buff = malloc(buff_len);
...
preq_partRSEQ1a:
    if (used + 4 > buff_len) {
        grow_buff(&buff, &buff_len);
        goto preq_partRSEQ1a;
    } else {
        if (fread(buff + used, 4, 1, f) != 1) return false;
        used += 4;
    }
    int32_t messages_len = be32toh(*( (( int32_t *) (buff + used)) - 1));
    int32_t messages_iter;
    for(messages_iter = 0; messages_iter < messages_len; ++messages_iter) {
    preq_partRSEQ1message0a:
        if (used + 2 > buff_len) {
            grow_buff(&buff, &buff_len);
            goto preq_partRSEQ1message0a;
        } else {
            if (fread(buff + used, 2, 1, f) != 1) return false;
            used += 2;
        }
        int16_t b_len = be16toh(*( (( int16_t *) (buff + used)) - 1));
    preq_partRSEQ1message0b:
        if (used + (b_len * 1) > buff_len) {
            grow_buff(&buff, &buff_len);
            goto preq_partRSEQ1message0b;
        } else {
            if ((b_len * 1) && fread(buff + used, (b_len * 1), 1, f) != 1) return false;
            used += (b_len * 1);
        }
    }
    int ret = preq_part_unpack_new(tgt, buff);
    free(buff);
    return ret;
}
void preq_part_free(preq_part *tgt)
{
    int32_t messages_iter = 0;
    for(messages_iter = 0; messages_iter < tgt->messages_len; ++messages_iter) {
        message_free(tgt->messages + messages_iter);
    }
    free(tgt->messages); tgt->messages = NULL;
}



typedef struct preq_topic {
    string topic_name;
    int32_t partitions_len;
    preq_part * partitions;
} preq_topic;

int preq_topic_size(const preq_topic const * b)
{
    int size = 4;
    size += string_size(&(b->topic_name));
    int32_t partitions_iter = 0;
    for(partitions_iter = 0; partitions_iter < b->partitions_len; ++partitions_iter) {
        size += preq_part_size(b->partitions + partitions_iter);
    }
    return size;
}
int preq_topic_pack(const preq_topic const *src, char *tgt)
{
    size_t bytes_written = 0;
    bytes_written += string_pack(&(src->topic_name), (tgt + bytes_written));
    *((int32_t*)(tgt + bytes_written)) = htobe32(src->partitions_len); bytes_written += 4;
    int32_t partitions_iter;
    for(partitions_iter = 0; partitions_iter < src->partitions_len; ++partitions_iter) {
        bytes_written += preq_part_pack(&(src->partitions[partitions_iter]), (tgt + bytes_written));
    }

    return bytes_written;
}
int preq_topic_unpack_new(preq_topic *tgt, const char const *src)
{
    size_t bytes_consumed = 0;
    bytes_consumed += string_unpack_new(&(tgt->topic_name), (src + bytes_consumed));
    int32_t partitions_iter = 0;
    tgt->partitions_len = be32toh(* ((int32_t*)(src + bytes_consumed))); bytes_consumed += 4;
    tgt->partitions = malloc(tgt->partitions_len * sizeof(preq_part));
    if (tgt->partitions_len < 0) tgt->partitions_len = 0;
    for(partitions_iter = 0; partitions_iter < tgt->partitions_len; ++partitions_iter) {
        bytes_consumed += preq_part_unpack_new(&(tgt->partitions[partitions_iter]), (src + bytes_consumed));
    }
...
int preq_topic_write(const preq_topic *src, FILE *f)
{
    int ret;
    size_t blk_size = preq_topic_size(src);
    char * buff = (char*) malloc(blk_size);
    if(!preq_topic_pack(src, buff)) return false;
    ret = fwrite(buff, blk_size, 1, f);
    free(buff);
    return ret;
}
int preq_topic_read_new(preq_topic *tgt, FILE *f)
{
    size_t buff_len = 1024;
    size_t used = 0;
    char *buff = malloc(buff_len);
...
preq_topicRSEQ1a:
    if (used + 4 > buff_len) {
        grow_buff(&buff, &buff_len);
        goto preq_topicRSEQ1a;
    } else {
        if (fread(buff + used, 4, 1, f) != 1) return false;
        used += 4;
    }
    int32_t partitions_len = be32toh(*( (( int32_t *) (buff + used)) - 1));
    int32_t partitions_iter;
    for(partitions_iter = 0; partitions_iter < partitions_len; ++partitions_iter) {
    preq_topicRSEQ1preq_part0:
        if (used + 4 > buff_len) {
            grow_buff(&buff, &buff_len);
            goto preq_topicRSEQ1preq_part0;
        } else {
            if (fread(buff + used, 4, 1, f) != 1) return false;
            used += 4;
        }
    preq_topicRSEQ1preq_part1a:
        if (used + 4 > buff_len) {
            grow_buff(&buff, &buff_len);
            goto preq_topicRSEQ1preq_part1a;
        } else {
            if (fread(buff + used, 4, 1, f) != 1) return false;
            used += 4;
        }
        ...
        int32_t messages_iter;
        for(messages_iter = 0; messages_iter < messages_len; ++messages_iter) {
        preq_topicRSEQ1preq_part1message0a:
            if (used + 2 > buff_len) {
                grow_buff(&buff, &buff_len);
                goto preq_topicRSEQ1preq_part1message0a;
            } else {
                if (fread(buff + used, 2, 1, f) != 1) return false;
                used += 2;
            }
            int16_t b_len = be16toh(*( (( int16_t *) (buff + used)) - 1));
        preq_topicRSEQ1preq_part1message0b:
            if (used + (b_len * 1) > buff_len) {
                grow_buff(&buff, &buff_len);
                goto preq_topicRSEQ1preq_part1message0b;
            } else {
                if ((b_len * 1) && fread(buff + used, (b_len * 1), 1, f) != 1) return false;
                used += (b_len * 1);
            }
    ...
    int ret = preq_topic_unpack_new(tgt, buff);
    free(buff);
    return ret;
}
void preq_topic_free(preq_topic *tgt)
{
    string_free(&(tgt->topic_name));
    int32_t partitions_iter = 0;
    for(partitions_iter = 0; partitions_iter < tgt->partitions_len; ++partitions_iter) {
        preq_part_free(tgt->partitions + partitions_iter);
    }
    free(tgt->partitions); tgt->partitions = NULL;
}



typedef struct produce_request {
...
int produce_request_size(const produce_request const * b)
{
    int size = 10;
    int32_t topics_iter = 0;
    for(topics_iter = 0; topics_iter < b->topics_len; ++topics_iter) {
        size += preq_topic_size(b->topics + topics_iter);
    }
    return size;
}
int produce_request_pack(const produce_request const *src, char *tgt)
{
    size_t bytes_written = 0;
    *((int16_t*)(tgt + bytes_written)) = htobe16(src->required_acks); bytes_written += 2;
    *((int32_t*)(tgt + bytes_written)) = htobe32(src->timeout); bytes_written += 4;
    *((int32_t*)(tgt + bytes_written)) = htobe32(src->topics_len); bytes_written += 4;
    int32_t topics_iter;
    for(topics_iter = 0; topics_iter < src->topics_len; ++topics_iter) {
        bytes_written += preq_topic_pack(&(src->topics[topics_iter]), (tgt + bytes_written));
    }

    return bytes_written;
}
int produce_request_unpack_new(produce_request *tgt, const char const *src)
{
    size_t bytes_consumed = 0;
    tgt->required_acks = be16toh(* ((int16_t*)(src + bytes_consumed))); bytes_consumed += 2;
    tgt->timeout = be32toh(* ((int32_t*)(src + bytes_consumed))); bytes_consumed += 4;
    int32_t topics_iter = 0;
    tgt->topics_len = be32toh(* ((int32_t*)(src + bytes_consumed))); bytes_consumed += 4;
    tgt->topics = malloc(tgt->topics_len * sizeof(preq_topic));
    if (tgt->topics_len < 0) tgt->topics_len = 0;
    for(topics_iter = 0; topics_iter < tgt->topics_len; ++topics_iter) {
        bytes_consumed += preq_topic_unpack_new(&(tgt->topics[topics_iter]), (src + bytes_consumed));
    }
...
int produce_request_write(const produce_request *src, FILE *f)
{
    int ret;
    size_t blk_size = produce_request_size(src);
    char * buff = (char*) malloc(blk_size);
    if(!produce_request_pack(src, buff)) return false;
    ret = fwrite(buff, blk_size, 1, f);
    free(buff);
    return ret;
}
int produce_request_read_new(produce_request *tgt, FILE *f)
{
    size_t buff_len = 1024;
    size_t used = 0;
    char *buff = malloc(buff_len);
produce_requestRSEQ0:
    if (used + 6 > buff_len) {
        grow_buff(&buff, &buff_len);
        goto produce_requestRSEQ0;
    } else {
        if (fread(buff + used, 6, 1, f) != 1) return false;
        used += 6;
    }
produce_requestRSEQ1a:
    if (used + 4 > buff_len) {
        grow_buff(&buff, &buff_len);
        goto produce_requestRSEQ1a;
    } else {
        if (fread(buff + used, 4, 1, f) != 1) return false;
        used += 4;
    }
    ...
    int32_t topics_iter;
    for(topics_iter = 0; topics_iter < topics_len; ++topics_iter) {
    produce_requestRSEQ1preq_topic0a:
        if (used + 2 > buff_len) {
            grow_buff(&buff, &buff_len);
            goto produce_requestRSEQ1preq_topic0a;
        } else {
            if (fread(buff + used, 2, 1, f) != 1) return false;
            used += 2;
        }
        int16_t s_len = be16toh(*( (( int16_t *) (buff + used)) - 1));
    produce_requestRSEQ1preq_topic0b:
        if (used + (s_len * 1) > buff_len) {
            grow_buff(&buff, &buff_len);
            goto produce_requestRSEQ1preq_topic0b;
        } else {
            if ((s_len * 1) && fread(buff + used, (s_len * 1), 1, f) != 1) return false;
            used += (s_len * 1);
        }
    produce_requestRSEQ1preq_topic1a:
        if (used + 4 > buff_len) {
            grow_buff(&buff, &buff_len);
            goto produce_requestRSEQ1preq_topic1a;
        } else {
            if (fread(buff + used, 4, 1, f) != 1) return false;
            used += 4;
        }
        ...
        int32_t partitions_iter;
        for(partitions_iter = 0; partitions_iter < partitions_len; ++partitions_iter) {
        produce_requestRSEQ1preq_topic1preq_part0:
            if (used + 4 > buff_len) {
                grow_buff(&buff, &buff_len);
                goto produce_requestRSEQ1preq_topic1preq_part0;
            } else {
                if (fread(buff + used, 4, 1, f) != 1) return false;
                used += 4;
            }
        produce_requestRSEQ1preq_topic1preq_part1a:
            if (used + 4 > buff_len) {
                grow_buff(&buff, &buff_len);
                goto produce_requestRSEQ1preq_topic1preq_part1a;
            } else {
                if (fread(buff + used, 4, 1, f) != 1) return false;
                used += 4;
            }
            ...
            int32_t messages_iter;
            for(messages_iter = 0; messages_iter < messages_len; ++messages_iter) {
            produce_requestRSEQ1preq_topic1preq_part1message0a:
                if (used + 2 > buff_len) {
                    grow_buff(&buff, &buff_len);
                    goto produce_requestRSEQ1preq_topic1preq_part1message0a;
                } else {
                    if (fread(buff + used, 2, 1, f) != 1) return false;
                    used += 2;
                }
                int16_t b_len = be16toh(*( (( int16_t *) (buff + used)) - 1));
            produce_requestRSEQ1preq_topic1preq_part1message0b:
                if (used + (b_len * 1) > buff_len) {
                    grow_buff(&buff, &buff_len);
                    goto produce_requestRSEQ1preq_topic1preq_part1message0b;
                } else {
                    if ((b_len * 1) && fread(buff + used, (b_len * 1), 1, f) != 1) return false;
                    used += (b_len * 1);
                }
    ...
    int ret = produce_request_unpack_new(tgt, buff);
    free(buff);
    return ret;
}
void produce_request_free(produce_request *tgt)
{
    int32_t topics_iter = 0;
    for(topics_iter = 0; topics_iter < tgt->topics_len; ++topics_iter) {
        preq_topic_free(tgt->topics + topics_iter);
    }
    free(tgt->topics); tgt->topics = NULL;
}
...
typedef struct pres_part {
    int32_t partition_no;
    int16_t partition_error_code;
    int64_t offset;
} pres_part;

int pres_part_size(const pres_part const * b)
{
    int size = 14;
    return size;
}
int pres_part_pack(const pres_part const *src, char *tgt)
{
    size_t bytes_written = 0;
    *((int32_t*)(tgt + bytes_written)) = htobe32(src->partition_no); bytes_written += 4;
    *((int16_t*)(tgt + bytes_written)) = htobe16(src->partition_error_code); bytes_written += 2;
    *((int64_t*)(tgt + bytes_written)) = htobe64(src->offset); bytes_written += 8;

    return bytes_written;
}
int pres_part_unpack_new(pres_part *tgt, const char const *src)
{
    size_t bytes_consumed = 0;
    tgt->partition_no = be32toh(* ((int32_t*)(src + bytes_consumed))); bytes_consumed += 4;
    tgt->partition_error_code = be16toh(* ((int16_t*)(src + bytes_consumed))); bytes_consumed += 2;
    tgt->offset = be64toh(* ((int64_t*)(src + bytes_consumed))); bytes_consumed += 8;
...
int pres_part_write(const pres_part *src, FILE *f)
{
    int ret;
    size_t blk_size = 14;
    char buff[14];
    if(!pres_part_pack(src, buff)) return false;
    ret = fwrite(buff, blk_size, 1, f);
    return ret;
}
int pres_part_read_new(pres_part *tgt, FILE *f)
{
    size_t blk_size = 14;
    char buff[blk_size];
    if (fread(buff, blk_size, 1, f) != 1) return false;
    return pres_part_unpack_new(tgt, buff);
}
void pres_part_free(pres_part *tgt)
{
}



typedef struct pres_topic {
    string topic_name;
    int32_t partitions_len;
    preq_part * partitions;
} pres_topic;

int pres_topic_size(const pres_topic const * b)
{
    int size = 4;
    size += string_size(&(b->topic_name));
    int32_t partitions_iter = 0;
...
int pres_topic_pack(const pres_topic const *src, char *tgt)
{
    size_t bytes_written = 0;
    bytes_written += string_pack(&(src->topic_name), (tgt + bytes_written));
    *((int32_t*)(tgt + bytes_written)) = htobe32(src->partitions_len); bytes_written += 4;
...
int pres_topic_unpack_new(pres_topic *tgt, const char const *src)
{
    size_t bytes_consumed = 0;
    bytes_consumed += string_unpack_new(&(tgt->topic_name), (src + bytes_consumed));
    int32_t partitions_iter = 0;
...
int pres_topic_write(const pres_topic *src, FILE *f)
{
    int ret;
    size_t blk_size = pres_topic_size(src);
    char * buff = (char*) malloc(blk_size);
    if(!pres_topic_pack(src, buff)) return false;
    ret = fwrite(buff, blk_size, 1, f);
    free(buff);
    return ret;
}
int pres_topic_read_new(pres_topic *tgt, FILE *f)
{
    size_t buff_len = 1024;
    size_t used = 0;
    char *buff = malloc(buff_len);
...
pres_topicRSEQ1a:
    ...
    int32_t partitions_iter;
    for(partitions_iter = 0; partitions_iter < partitions_len; ++partitions_iter) {
    pres_topicRSEQ1preq_part0:
        if (used + 4 > buff_len) {
            grow_buff(&buff, &buff_len);
            goto pres_topicRSEQ1preq_part0;
        } else {
            if (fread(buff + used, 4, 1, f) != 1) return false;
            used += 4;
        }
    pres_topicRSEQ1preq_part1a:
        if (used + 4 > buff_len) {
            grow_buff(&buff, &buff_len);
            goto pres_topicRSEQ1preq_part1a;
        } else {
            if (fread(buff + used, 4, 1, f) != 1) return false;
            used += 4;
        }
        ...
        int32_t messages_iter;
        for(messages_iter = 0; messages_iter < messages_len; ++messages_iter) {
        pres_topicRSEQ1preq_part1message0a:
            if (used + 2 > buff_len) {
                grow_buff(&buff, &buff_len);
                goto pres_topicRSEQ1preq_part1message0a;
            } else {
                if (fread(buff + used, 2, 1, f) != 1) return false;
                used += 2;
            }
            int16_t b_len = be16toh(*( (( int16_t *) (buff + used)) - 1));
        pres_topicRSEQ1preq_part1message0b:
            if (used + (b_len * 1) > buff_len) {
                grow_buff(&buff, &buff_len);
                goto pres_topicRSEQ1preq_part1message0b;
            } else {
                if ((b_len * 1) && fread(buff + used, (b_len * 1), 1, f) != 1) return false;
                used += (b_len * 1);
            }
    ...
    int ret = pres_topic_unpack_new(tgt, buff);
    free(buff);
    return ret;
}
void pres_topic_free(pres_topic *tgt)
{
    string_free(&(tgt->topic_name));
    int32_t partitions_iter = 0;
    for(partitions_iter = 0; partitions_iter < tgt->partitions_len; ++partitions_iter) {
...
typedef struct produce_response {
    int32_t topics_len;
    pres_topic * topics;
} produce_response;

int produce_response_size(const produce_response const * b)
{
    int size = 4;
    int32_t topics_iter = 0;
    for(topics_iter = 0; topics_iter < b->topics_len; ++topics_iter) {
        size += pres_topic_size(b->topics + topics_iter);
    }
    return size;
}
int produce_response_pack(const produce_response const *src, char *tgt)
{
    size_t bytes_written = 0;
    *((int32_t*)(tgt + bytes_written)) = htobe32(src->topics_len); bytes_written += 4;
    int32_t topics_iter;
    for(topics_iter = 0; topics_iter < src->topics_len; ++topics_iter) {
        bytes_written += pres_topic_pack(&(src->topics[topics_iter]), (tgt + bytes_written));
    }

    return bytes_written;
}
int produce_response_unpack_new(produce_response *tgt, const char const *src)
{
    size_t bytes_consumed = 0;
    int32_t topics_iter = 0;
    tgt->topics_len = be32toh(* ((int32_t*)(src + bytes_consumed))); bytes_consumed += 4;
    tgt->topics = malloc(tgt->topics_len * sizeof(pres_topic));
    if (tgt->topics_len < 0) tgt->topics_len = 0;
    for(topics_iter = 0; topics_iter < tgt->topics_len; ++topics_iter) {
        bytes_consumed += pres_topic_unpack_new(&(tgt->topics[topics_iter]), (src + bytes_consumed));
    }
...
int produce_response_write(const produce_response *src, FILE *f)
{
    int ret;
    size_t blk_size = produce_response_size(src);
    char * buff = (char*) malloc(blk_size);
    if(!produce_response_pack(src, buff)) return false;
    ret = fwrite(buff, blk_size, 1, f);
    free(buff);
    return ret;
}
int produce_response_read_new(produce_response *tgt, FILE *f)
{
    size_t buff_len = 1024;
    size_t used = 0;
    char *buff = malloc(buff_len);
produce_responseRSEQ0a:
    if (used + 4 > buff_len) {
        grow_buff(&buff, &buff_len);
        goto produce_responseRSEQ0a;
    } else {
        if (fread(buff + used, 4, 1, f) != 1) return false;
        used += 4;
    }
    ...
    int32_t topics_iter;
    for(topics_iter = 0; topics_iter < topics_len; ++topics_iter) {
    produce_responseRSEQ0pres_topic0a:
        if (used + 2 > buff_len) {
            grow_buff(&buff, &buff_len);
            goto produce_responseRSEQ0pres_topic0a;
        } else {
            if (fread(buff + used, 2, 1, f) != 1) return false;
            used += 2;
        }
        int16_t s_len = be16toh(*( (( int16_t *) (buff + used)) - 1));
    produce_responseRSEQ0pres_topic0b:
        if (used + (s_len * 1) > buff_len) {
            grow_buff(&buff, &buff_len);
            goto produce_responseRSEQ0pres_topic0b;
        } else {
            if ((s_len * 1) && fread(buff + used, (s_len * 1), 1, f) != 1) return false;
            used += (s_len * 1);
        }
    produce_responseRSEQ0pres_topic1a:
        if (used + 4 > buff_len) {
            grow_buff(&buff, &buff_len);
            goto produce_responseRSEQ0pres_topic1a;
        } else {
            if (fread(buff + used, 4, 1, f) != 1) return false;
            used += 4;
        }
        ...
        int32_t partitions_iter;
        for(partitions_iter = 0; partitions_iter < partitions_len; ++partitions_iter) {
        produce_responseRSEQ0pres_topic1preq_part0:
            if (used + 4 > buff_len) {
                grow_buff(&buff, &buff_len);
                goto produce_responseRSEQ0pres_topic1preq_part0;
            } else {
                if (fread(buff + used, 4, 1, f) != 1) return false;
                used += 4;
            }
        produce_responseRSEQ0pres_topic1preq_part1a:
            if (used + 4 > buff_len) {
                grow_buff(&buff, &buff_len);
                goto produce_responseRSEQ0pres_topic1preq_part1a;
            } else {
                if (fread(buff + used, 4, 1, f) != 1) return false;
                used += 4;
            }
            ...
            int32_t messages_iter;
            for(messages_iter = 0; messages_iter < messages_len; ++messages_iter) {
            produce_responseRSEQ0pres_topic1preq_part1message0a:
                if (used + 2 > buff_len) {
                    grow_buff(&buff, &buff_len);
                    goto produce_responseRSEQ0pres_topic1preq_part1message0a;
                } else {
                    if (fread(buff + used, 2, 1, f) != 1) return false;
                    used += 2;
                }
                int16_t b_len = be16toh(*( (( int16_t *) (buff + used)) - 1));
            produce_responseRSEQ0pres_topic1preq_part1message0b:
                if (used + (b_len * 1) > buff_len) {
                    grow_buff(&buff, &buff_len);
                    goto produce_responseRSEQ0pres_topic1preq_part1message0b;
                } else {
                    if ((b_len * 1) && fread(buff + used, (b_len * 1), 1, f) != 1) return false;
                    used += (b_len * 1);
                }
    ...
    int ret = produce_response_unpack_new(tgt, buff);
    free(buff);
    return ret;
}
void produce_response_free(produce_response *tgt)
{
    int32_t topics_iter = 0;
    for(topics_iter = 0; topics_iter < tgt->topics_len; ++topics_iter) {
        pres_topic_free(tgt->topics + topics_iter);
    }
    free(tgt->topics); tgt->topics = NULL;
}
